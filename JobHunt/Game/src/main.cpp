...
int APIENTRY wWinMain(HINSTANCE hInstance,HINSTANCE,LPWSTR,int nCmdShow){
    ...
    HWND hwnd=CreateWindowExW(0,wc.lpszClassName,L"Dx12 + Engine Prototype (V5)",
    ...
    gRenderer=CreateRenderer(); if(!gRenderer->Initialize(hwnd,gClientWidth,gClientHeight)){ DestroyRenderer(gRenderer); MessageBoxW(nullptr,L"Renderer init failed",L"Error",MB_ICONERROR); return -1; }
    ...
    MSG msg{}; bool running=true; while(running){
        ...
        if(!running) break; auto now=std::chrono::high_resolution_clock::now(); std::chrono::duration<double> dt=now-prev; prev=now; gRenderer->Update((float)dt.count()); gRenderer->Render();
    }
    gRenderer->Shutdown(); DestroyRenderer(gRenderer); gRenderer=nullptr; return 0;
}
